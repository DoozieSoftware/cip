...
**Version:** 1.0
**Status:** In Progress (14 / 24 tasks complete; T-M3-015 next)
**Last updated:** 2026-06-27 05:00 IST (after T-M3-014 done)
...
* Active task: **T-M3-015 — GeographyRepository and GeographyService** (UUID PK, name, unique code, parent_id self-FK, jurisdiction, address, email, phone, working_hours JSON, holiday_calendar JSON, default_workflow_id, default_sla_minutes, escalation_matrix JSON, active, soft deletes).
* Blockers: none.
* Next task on completion: T-M3-016 (Department CRUD endpoints).
