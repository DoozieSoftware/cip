...
| P2-14 | **Partial** | Privacy-safe, allowlisted telemetry endpoint and browser fire-and-forget client are implemented; current producers cover landing/report start, step views, completion, and offline queue, while GPS/media failure, notification delivery, reopen, accessibility preference, and performance call sites remain to be wired. |
