...
test.describe('E2E-7 citizen reschedule happy [OPEN D-04]', () => {
  test.fixme(
    true,
    'BE-X5-RS1: citizen reschedules an eligible scheduled pickup before cutoff → new date, old batch atomically detached, audit shows old+new',
  );
  test.fixme(
    true,
    'BE-X5-RS11: history timeline shows old and new scheduled_date / batch / window',
  );
...
test.describe('E2E-8 reschedule cutoff [OPEN D-04]', () => {
  test.fixme(
    true,
    'BE-X5-RS2: reschedule within cutoff window → 422 RESCHEDULE_CUTOFF_PASSED with hint',
  );
  test.fixme(
    true,
    'BE-X5-RS4: citizen reschedule frozen when trip is in_progress; partner override path per D-04 role succeeds',
  );
});
...
test.describe('E2E-9 unavailable slot fallback [OPEN D-04]', () => {
  test.fixme(
    true,
    'BE-X5-RS8: requesting a no-longer-available slot → 422 SLOT_UNAVAILABLE with fallback note',
  );
  test.fixme(
    true,
    'Partner staff sees why the slot became unavailable (over-capacity / zone/day closed) on the queue',
  );
});
...
test.describe('E2E-10 duplicate booking protection [OPEN D-04]', () => {
  test.fixme(
    true,
    'Repeatedly rescheduling keeps exactly one active collection request (no duplicate active bookings)',
  );
});
...
test.describe('E2E-11 reminders [OPEN D-04]', () => {
  test.fixme(
    true,
    'BE-N-RS1: scheduled premises receives exactly one reminder before the trip date',
  );
  test.fixme(
    true,
    'BE-N-RS2: reminder never sent for cancelled / rejected / already-collected requests',
  );
  test.fixme(
    true,
    'Reminder respects opted-out / consent-withdrawn citizen preference and does not leak staff PII',
  );
});
...
test.describe('reschedule smoke (unblocked, no D-04)', () => {
  test('citizen detail shell still renders reschedule section without crashing', async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test('citizen collections list and detail remain reachable without the reschedule lane', async ({
    page,
  }) => {
  ...
  test('detail page on 375px viewport does not horizontally overflow reschedule section', async ({
    page,
  }) => {
