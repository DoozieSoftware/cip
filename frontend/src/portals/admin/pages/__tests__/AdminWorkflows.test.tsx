...
const WFS = [
  {
    ...
    transitions: [
      ...
      {
        id: 't2',
        from_state_id: 'triaged',
        to_state_id: 'resolved',
        event: 'resolve',
        required_role: 'department_officer',
      },
    ],
  },
];
...
describe('AdminWorkflows (T-M12-019)', () => {
  ...
  beforeEach(() => {
    ...
    (useWorkflows as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
      data: WFS,
      isLoading: false,
    });
  ...
  it('reveals the transition matrix when show matrix is clicked', async () => {
    ...
    btn.click();
    expect(await screen.findByText('Transition matrix — Report lifecycle')).toBeTruthy();
    expect(screen.getByText('resolved (terminal)')).toBeTruthy();
  });
});
