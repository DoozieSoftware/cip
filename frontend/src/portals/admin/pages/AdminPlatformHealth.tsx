  ...
  return (
    <span
      className={cx(
        ...
        STATUS_COLOR[status] ?? 'bg-[var(--color-surface-alt)] text-[var(--color-text-secondary)] ring-[var(--color-border)]',
      )}
    >
                  ...
                  {Object.entries(rows).map(([key, comp]) => (
                    <tr key={key}>
                      ...
                      <td className="px-5 py-3 text-sm text-[var(--color-text-secondary)]">
                        <div>{comp.detail}</div>
                        {comp.driver ? (
                          <div className="text-xs text-[var(--color-text-tertiary)]">driver: {comp.driver}</div>
                        ) : null}
                        {comp.disk ? (
                          <div className="text-xs text-[var(--color-text-tertiary)]">disk: {comp.disk}</div>
                        ) : null}
                        {typeof comp.count === 'number' ? (
                          <div className="text-xs text-[var(--color-text-tertiary)]">count: {comp.count}</div>
                        ) : null}
                      </td>
                    </tr>
                  ))}
