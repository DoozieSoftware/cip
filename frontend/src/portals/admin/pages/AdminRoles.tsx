...
export default function AdminRoles(): JSX.Element {
  ...
  const toDraft = (r: AdminRole | null): RoleDraft =>
    r ? { id: r.id, name: r.name, guard_name: r.guard_name, permissions: r.permissions } : blank;
  ...
  const onSubmit = (v: AdminRoleInput & { id?: string | number }): void => {
    ...
    if (id === undefined) {
      ...
      update.mutate(
        { id: String(id), name: body.name, guard_name: body.guard_name },
        {
          onSuccess: () => {
            sync.mutate(
              { id: String(id), permissions: body.permissions ?? [] },
              { onSuccess: close },
            );
          },
        },
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
        ...
        and similar — stay governed by a user&rsquo;s role and their department membership;
