...
export const GpsCapture = forwardRef<GpsCaptureHandle, GpsCaptureProps>(function GpsCapture(props, ref): JSX.Element {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          if (err.code === 1) {
            ...
            setError('Location access is blocked. Enable it in your phone and browser settings, then try again.');
          } else if (err.code === 3) {
            setError('Location lookup timed out. Move to an open area and try again.');
          } else {
            setError('Your location is unavailable. Check that Location Services are on, then try again.');
          }
          setBusy(false);
          resolve(null);
        },
  ...
  return (
    <div className={cx('space-y-2', className)}>
      ...
      {error ? (
        <div role="alert" className="rounded-md border border-rose-300 bg-rose-50 px-3 py-3 text-sm text-rose-900">
