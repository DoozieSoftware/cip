  ...
  if (isLoading) {
    return (
      <div
        ...
        className={`flex items-center gap-2 rounded-xl border border-[var(--color-border-subtle)] bg-white px-4 py-3 text-xs text-[var(--color-text-secondary)] ${className ?? ''}`}
      >
    ...
    return (
      ...
      >
        <p className="flex items-center gap-1.5 text-sm font-semibold text-amber-800">
          <IconAlertTriangle className="h-4 w-4" stroke={1.65} />
        ...
        {onRetry ? (
          <button
            type="button"
            onClick={onRetry}
            className="mt-2 inline-flex min-h-11 items-center rounded-full border border-amber-300 bg-white px-4 text-xs font-medium text-amber-800"
          >
            Retry
          </button>
        ) : null}
    ...
    <div className={`space-y-2 ${className ?? ''}`}>
          ...
          {evaluation.totals ? (
            <p className="mt-1 text-xs text-rose-700">
              ...
              {evaluation.effective_rule
                ? ` · Limit: ${evaluation.effective_rule.max_bags ?? '—'} bags / ${evaluation.effective_rule.max_weight_kg ?? '—'} kg / ${evaluation.effective_rule.max_stops ?? '—'} stops`
                : ''}
            </p>
          ) : null}
          <ul className="mt-2 space-y-1.5">
            {blockers.map((item) => (
              <li key={item.code} className="text-sm leading-5 text-rose-800">
                {item.message}
              </li>
            ))}
          </ul>
          {evaluation.effective_rule?.guidance_text ? (
            <p className="mt-2 text-xs text-rose-700">{evaluation.effective_rule.guidance_text}</p>
          ) : null}
          {onRequestException ? (
            <div className="mt-3">
              <button
                ...
                className="inline-flex min-h-11 items-center rounded-full bg-rose-700 px-5 text-xs font-medium text-white disabled:opacity-40"
              >
      ...
      {hasWarnings ? (
        <div role="status" className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          <p className="flex items-center gap-1.5 text-sm font-semibold text-amber-800">
            <IconInfoCircle className="h-4 w-4" stroke={1.65} />
          ...
          <ul className="mt-2 space-y-1.5">
            {warnings.map((item) => (
              <li key={item.code} className="text-sm leading-5 text-amber-800">
                {item.message}
              </li>
            ))}
          </ul>
          {evaluation.effective_rule?.guidance_text ? (
            <p className="mt-2 text-xs text-amber-700">{evaluation.effective_rule.guidance_text}</p>
          ) : null}
          {onRequestException ? (
            <div className="mt-3">
              <button
                ...
                disabled={!!isRequestingException}
                onClick={onRequestException}
                className="inline-flex min-h-11 items-center rounded-full border border-amber-300 bg-white px-5 text-xs font-medium text-amber-800 disabled:opacity-40"
              >
                {isRequestingException ? 'Requesting…' : requestExceptionLabel}
              </button>
              <p className="mt-1 text-xs text-amber-700">
