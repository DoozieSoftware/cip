...
const makeItem = (
  overrides: Partial<TextileCollectionListItem> = {},
...
describe('Textile trip manifest (Phase 2)', () => {
  ...
  it('manifest shows trip progress badge (unstarted / in progress / completed states)', () => {
    const planned = makeItem({
      batch: {
        id: 'b1',
        reference: 'DRL-260826-AA',
        collection_date: '2026-08-27',
        status: 'planned',
      },
    });
    const inProg = makeItem({
      id: 'c-2',
      reference: 'DLN-2026-BBBB2222',
      batch: {
        id: 'b1',
        reference: 'DRL-260826-AA',
        collection_date: '2026-08-27',
    ...
    vi.mocked(useTextileQueue).mockReturnValue({
      data: { data: [planned, inProg], meta: { page: 1, per_page: 25, total: 2, last_page: 1 } },
      isLoading: false,
      isError: false,
      refetch: vi.fn(),
    } as unknown as ReturnType<typeof useTextileQueue>);
  ...
  it('manifest renders loading / empty / error states (AGENTS.md frontend rule)', () => {
    ...
    vi.mocked(useTextileQueue).mockReturnValue({
      data: { data: [], meta: { page: 1, per_page: 25, total: 0, last_page: 1 } },
      isLoading: false,
      isError: false,
      refetch: vi.fn(),
    } as unknown as ReturnType<typeof useTextileQueue>);
  ...
  it('missed stop shows citizen-visible explanation and re-scheduling cue', () => {
    const missed = makeItem({
      status: 'missed',
      missed_pickup_reason: 'Gate locked, nobody at home',
    });
    vi.mocked(useTextileQueue).mockReturnValue({
      data: { data: [missed], meta: { page: 1, per_page: 25, total: 1, last_page: 1 } },
      isLoading: false,
      isError: false,
      refetch: vi.fn(),
    } as unknown as ReturnType<typeof useTextileQueue>);
