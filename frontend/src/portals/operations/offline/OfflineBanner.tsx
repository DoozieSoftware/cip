  ...
  return (
    <div className="space-y-2">
      {!isOnline ? (
        <div
          role="status"
          aria-label="Offline — collections will be queued"
        ...
        >
          ...
          <p className="text-xs font-medium text-amber-800">
            You are offline — new collections will be queued and uploaded when you are back online.
            Nothing is lost.
          </p>
        </div>
      ) : null}
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-medium text-sky-800">
              {pendingCount} pending upload{pendingCount === 1 ? '' : 's'} — will retry
          ...
          <button
            type="button"
            onClick={() => void drain()}
            className="inline-flex min-h-8 items-center gap-1 rounded-full border border-sky-300 bg-white px-3 text-xs font-medium text-sky-800"
          >
      ...
      {deadCount > 0 ? (
        <div
          role="alert"
          aria-label={`${deadCount} failed upload${deadCount === 1 ? '' : 's'} need attention`}
