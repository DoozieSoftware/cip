 ...
 *
 * TEST/DEV ONLY — the provider factory never selects this class
 * (it was removed from runtime wiring in the "remove all mocks and
 * stubs" refactor), but the benchmark and provider test suites
 * still exercise the AIProviderInterface contract through it.
 * Do not register it in config/ai.php.
 *
 * Behaviour:
 ...
 *    injected responses map (or falls back to the `default`
 *    entry, then to an honest low-confidence "uncategorised").
 *    No HTTP, no randomness — the same input always produces
 *    the same response, which is what the benchmark suite and
 *    the ProviderFailoverService tests depend on.
 */
