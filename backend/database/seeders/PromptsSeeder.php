...
class PromptsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($prompts as $p) {
            if ($p['name'] === 'category_classifier') {
                $p['prompt_text'] .= self::WASTE_STREAM_PROMPT_BLOCK."\n\n## Phase 1 routing signals\n\nReturn `emergency_flag` as true only for an immediate life-safety risk such as fire, collapse, live wire, trapped person, severe flooding, or a major crash. Return false for routine issues.\n\nReturn `secondary_triggers` as an array using only these codes when a second agency is needed: traffic_obstruction, road_damage_by_utility_work, sewage_in_drain, cable_hazard, footpath_damage_by_parking. Return [] when no co-routing is needed.";
            }
