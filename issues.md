...
| 6 | Add provisions for collecting clothes and metal waste, and discarding electronic waste. | Verified — ready to implement | Full checklist compiled: new `report_types` rows (codes/names/icons/evidence flags), routing rules (clothes/metal fit `BBMP_SWM`’s existing dry-waste mandate; **e-waste has no owner today**), AI classifier prompt bump (new codes are invisible to the model until a new prompt version is seeded — codes are embedded at seed time), frontend icons + picker order, docs updates, tests. Migration must be self-sufficient (prod runs `migrate --force` only, no `db:seed`). Awaiting decisions D10–D15. |
